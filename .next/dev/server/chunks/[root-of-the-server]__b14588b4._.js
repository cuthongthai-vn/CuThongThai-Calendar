...
async function GET() {
    ...
    const { data, error } = await supabase.from('economic_events').select('*').order('event_time', {
        ascending: false
    });
    if (error) {
        return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
...
async function POST(request) {
    ...
    try {
        ...
        const { data, error } = await supabase.from('economic_events').insert([
            body
        ]).select();
...
//# sourceMappingURL=%5Broot-of-the-server%5D__b14588b4._.js.map